...
const FeatureFallback = (function() {
  ...
  function init() {
    ...
    // Überwache Features periodisch
    if (CONFIG.enableFallbacks) {
      setInterval(checkAllFeatures, 30000); // Alle 30 Sekunden prüfen
    }
  }
  ...
  function checkAllFeatures() {
    ...
    featureStatus.botPersonalities = checkBotPersonalities();
    featureStatus.contextualOCR = checkContextualOCR();
    featureStatus.multiScoreDetection = checkMultiScoreDetection();
    
    featureStatus.allFeaturesReady = Object.values(featureStatus).every(status => status === true);
  ...
  /**
   * Prüft Bot Personalities
   */
  function checkBotPersonalities() {
    try {
      if (typeof BotPersonalities === 'undefined') return false;
      
      // Teste grundlegende Funktionen
      const personalities = BotPersonalities.PERSONALITIES;
      const current = BotPersonalities.getCurrentPersonality();
      
      return typeof personalities === 'object' && 
             (current === null || typeof current === 'object');
    } catch (error) {
      console.warn('⚠️ Bot Personalities Fehler:', error);
      return false;
    }
  }
  ...
  function setupErrorHandlers() {
    ...
    window.addEventListener('error', function(event) {
      ...
      } else if (errorMessage.includes('BotPersonalities') || errorStack.includes('bot-personalities')) {
        handleFeatureError('botPersonalities');
  ...
  function disableFeature(featureName) {
    switch (featureName) {
      ...
      case 'botPersonalities':
        if (typeof BotPersonalities !== 'undefined') {
          BotPersonalities.useClassicMode();
          console.log('🔄 Bot Personalities deaktiviert');
        }
        break;
  ...
  function showFeatureErrorNotification(featureName) {
    const featureNames = {
      ...
      botPersonalities: 'Bot-Persönlichkeiten',
  ...
  function runSelfTest() {
    ...
    const testResults = {
      ...
      botPersonalities: testBotPersonalities(),
  ...
  /**
   * Testet Bot Personalities
   */
  function testBotPersonalities() {
    try {
      if (typeof BotPersonalities === 'undefined') return 'nicht verfügbar';
      
      const personalities = BotPersonalities.PERSONALITIES;
      return personalities && Object.keys(personalities).length > 0 ? 'funktionsfähig' : 'Keine Persönlichkeiten geladen';
    } catch (error) {
      return `Fehler: ${error.message}`;
    }
  }
